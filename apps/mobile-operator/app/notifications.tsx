...
export default function NotificationsScreen() {
  const { user } = useAuth();
  const notificationRole: 'affiliate' | 'driver' =
    user?.role === 'affiliate' ? 'affiliate' : 'driver';
  const [notifications, setNotifications] = useState<Notification[]>([]);
  const [loading, setLoading] = useState(true);

  const load = async () => {
    const data = await notificationService.getNotifications(notificationRole);
  ...
  const handleMarkRead = async (id: string) => {
    await notificationService.markAsRead(notificationRole, id);
    setNotifications((prev) => prev.map((n) => n.id === id ? { ...n, isRead: true } : n));
  };

  const handleMarkAllRead = async () => {
    await notificationService.markAllAsRead(notificationRole);
