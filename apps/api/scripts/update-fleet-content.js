/**
 * One-off content push: applies the refreshed minibus/coach marketing copy,
 * specs, and bundled image URLs to the live database.
 *
 * Run from apps/api: node scripts/update-fleet-content.js
 */
...
const vehicleUpdates = [
  {
    ...
    imageUrl: '/images/fleet/minibus-16.webp',
  },
  {
    id: 'wv-m2',
    ...
    imageUrl: '/images/fleet/minibus-24.webp',
  },
  {
    id: 'wv-m3',
    imageUrl: '/images/fleet/midi-coach-32.webp',
  },
  {
    id: 'wv-c1',
    ...
    imageUrl: '/images/fleet/coach-44.webp',
  },
  {
    id: 'wv-c2',
    imageUrl: '/images/fleet/coach-53.webp',
  },
  {
    id: 'wv-c3',
    imageUrl: '/images/fleet/coach-60.webp',
  },
];
...
async function main() {
  for (const { id, ...data } of vehicleUpdates) {
    const result = await prisma.websiteVehicle.updateMany({ where: { id }, data });
    console.log(`${result.count ? 'Updated' : 'Skipped missing'} WebsiteVehicle ${id}`);
  }

  for (const { id, ...data } of categoryUpdates) {
    const result = await prisma.websiteFleetCategory.updateMany({ where: { id }, data });
    console.log(`${result.count ? 'Updated' : 'Skipped missing'} WebsiteFleetCategory ${id}`);
  }
}
