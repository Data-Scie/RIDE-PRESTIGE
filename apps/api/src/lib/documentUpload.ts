...
export function validateDocumentFile(file: UploadedFile): string | null {
...
export async function storeDocumentFile(req: Request, file: UploadedFile, ownerId: string, documentType: string): Promise<string> {
  const ext = path.extname(file.originalname).toLowerCase();
