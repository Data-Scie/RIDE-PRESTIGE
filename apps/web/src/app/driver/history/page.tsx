...
export default function DriverHistoryPage() {
  ...
  const total = rides.reduce((sum, ride) => sum + (ride.yourEarnings ?? 0), 0);

  return (
    <div className="space-y-6 max-w-6xl">
      <div className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-2xl font-bold text-slate-800">Ride History</h1><p className="text-sm text-slate-500">Completed and cancelled assignments</p></div>{total > 0 && <div className="text-right"><p className="text-2xl font-bold text-slate-800">£{total.toFixed(2)}</p><p className="text-xs text-slate-400">Total direct earnings</p></div>}</div>
          ...
          <div key={ride.id} className="p-5 hover:bg-slate-50/60 transition-colors">
            <div className="flex flex-wrap items-start gap-4">
              ...
              <div className="flex gap-5 text-right">
                ...
                {ride.yourEarnings !== null && ride.yourEarnings !== undefined && <div><p className="text-xs text-slate-400">Payout</p><p className="text-lg font-bold text-slate-800">£{ride.yourEarnings}</p></div>}
              </div>
            </div>
