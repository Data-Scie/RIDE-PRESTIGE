...
export default function AdminPromotionsPage() {
  ...
  useEffect(() => {
    adminApi.get<{ success: boolean; data: Promotion[] }>('/api/admin/promotions')
  ...
  const startNew = () => {
    setEditing({
      id: '', title: '', description: '', couponCode: '',
  ...
  const saveEdit = async () => {
    ...
    try {
      if (isNew) {
        ...
        const r = await adminApi.post<{ success: boolean; data: Promotion }>('/api/admin/promotions', body);
        setPromos(prev => [...prev, r.data]);
      } else {
        const r = await adminApi.put<{ success: boolean; data: Promotion }>(`/api/admin/promotions/${editing.id}`, editing);
  ...
  const deletePromo = async (id: string) => {
    await adminApi.delete(`/api/admin/promotions/${id}`).catch(() => {});
  ...
  const toggleActive = async (id: string) => {
    ...
    if (!p) return;
    const r = await adminApi.put<{ success: boolean; data: Promotion }>(`/api/admin/promotions/${id}`, { active: !p.active }).catch(() => null);
    if (r) setPromos(prev => prev.map(x => x.id === id ? r.data : x));
    else setPromos(prev => prev.map(x => x.id === id ? { ...x, active: !x.active } : x));
  };

  const setField = (key: keyof Promotion, value: unknown) => setEditing(prev => prev ? { ...prev, [key]: value } : null);

  if (loading) return <div className="flex items-center justify-center h-64 text-slate-400">Loading promotions…</div>;
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-luxury w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex gap-3 p-6 border-t border-gray-100 sticky bottom-0 bg-white rounded-b-3xl">
              <button onClick={saveEdit} disabled={saving || !editing.title}
                className="btn-gold flex items-center gap-2 flex-1 justify-center py-3 disabled:opacity-50">
                <Save size={15} />
                {saving ? 'Saving…' : isNew ? 'Add promotion' : 'Save changes'}
              </button>
