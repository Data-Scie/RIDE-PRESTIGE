...
export default function AdminRefundPage() {
  const [policy, setPolicy] = useState<CancellationPolicy>(DEFAULT);
  const [loading, setLoading] = useState(true);
  const [saved, setSaved] = useState(false);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    adminApi.get<{ success: boolean; data: CancellationPolicy }>('/api/admin/refund')
  ...
  const save = async () => {
    setSaving(true);
    await adminApi.put('/api/admin/refund', policy).catch(() => {});
    setSaving(false);
    setSaved(true);
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-7 space-y-5">
        <div>
          ...
          <input
            ...
            value={policy.minHoursBeforeRide}
        ...
        <div>
          ...
          <input
            type="number" step="1" min="0"
            value={policy.refundWindowHours}
