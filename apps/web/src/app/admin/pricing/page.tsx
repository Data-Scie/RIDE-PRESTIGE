...
export default function AdminPricingPage() {
  ...
  const save = async () => {
    ...
    setSaveError(null);
    try {
      await adminApi.put('/api/admin/pricing', pricing);
      setSaved(true);
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      {/* Save */}
      <div className="flex items-center justify-between bg-white rounded-2xl border border-gray-100 p-5 gap-4">
        <div>
          <p className="text-sm text-brand-grey">Changes are published live — new bookings use the updated rates immediately.</p>
          {saveError && <p className="text-xs text-red-600 mt-1">{saveError}</p>}
        </div>
        <button type="button" onClick={save} disabled={saving} className="btn-gold flex items-center gap-2 py-2.5 px-6 disabled:opacity-60 shrink-0">
