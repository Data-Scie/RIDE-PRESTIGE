...
export default function AdminNavigationPage() {
  ...
  useEffect(() => {
    adminApi.get<{ success: boolean; data: NavigationItem[] }>('/api/admin/navigation')
  ...
  const persistNav = async (updated: NavigationItem[]) => {
    await adminApi.put('/api/admin/navigation', { items: updated }).catch(() => {});
  };

  const toggleVisible = async (id: string) => {
    const u = nav.map(n => n.id === id ? { ...n, visible: !n.visible } : n);
    setNav(u);
    await persistNav(u);
  };
  ...
  if (loading) return <div className="flex items-center justify-center h-64 text-slate-400">Loading navigation…</div>;
      ...
      <div className="space-y-3">
        ...
        {nav.length === 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center text-brand-grey text-sm">
            No navigation items. Click &ldquo;Add item&rdquo; to create one.
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-luxury w-full max-w-md">
            ...
            <div className="flex gap-3 p-6 border-t border-gray-100">
              <button onClick={saveEdit} disabled={saving || !editing.label || !editing.href}
