...
export default function AdminVehiclesPage() {
  ...
  const saveEdit = async () => {
    ...
    try {
      if (isNew) {
        ...
        const r = await adminApi.post<{ success: boolean; data: Vehicle }>('/api/admin/fleet', body);
        setVehicles(prev => [...prev, r.data]);
      } else {
        const r = await adminApi.put<{ success: boolean; data: Vehicle }>(`/api/admin/fleet/${vehicle.id}`, vehicle);
        setVehicles(prev => prev.map(v => v.id === vehicle.id ? r.data : v));
      }
      setEditing(null);
      setIsNew(false);
      setSaved(true);
  ...
  const toggleAvailability = async (id: string) => {
    ...
    if (!v) return;
    await adminApi.put(`/api/admin/fleet/${id}`, { available: !v.available }).catch(() => {});
    setVehicles(prev => prev.map(x => x.id === id ? { ...x, available: !x.available } : x));
  };
