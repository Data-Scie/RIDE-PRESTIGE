    ...
    <div className="space-y-5 max-w-7xl">
      ...
      <div className="bg-white rounded-2xl p-4 flex flex-wrap gap-3" style={{ border: '1px solid #f0f0f0' }}>
        ...
        {[
          ...
          <button key={key} onClick={() => setFilter(key)} className="px-3 py-2 rounded-xl text-xs font-semibold" style={{ background: filter === key ? '#c9a84c' : '#f8fafc', color: filter === key ? 'white' : '#64748b' }}>{label}</button>
        ))}
      </div>
