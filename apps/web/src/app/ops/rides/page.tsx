...
const STATUS_COLORS: Record<string, string> = {
  awaiting_affiliate: '#f59e0b',
  needs_allocation: '#f59e0b',
...
function driverTypeLabel(type?: string | null) {
  if (type === 'independentDriver') return 'Independent';
  if (type === 'affiliateDriver') return 'Affiliate driver';
  return '';
}
...
export default function OpsRidesPage() {
  ...
  const filtered = jobs.filter(job => {
    ...
    const matchStatus = statusFilter === 'all' || (statusFilter === 'in_progress' ? activeStatuses.includes(job.status) : job.status === statusFilter);
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-48">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by ref, customer, affiliate, driver or vehicle" className="w-full pl-9 pr-4 py-2.5 rounded-xl text-sm outline-none border border-slate-200 focus:border-blue-400" />
        </div>
        <div className="flex flex-wrap gap-2">
          {['all', 'awaiting_affiliate', 'needs_allocation', 'in_progress', 'completed', 'cancelled'].map(status => (
            <button key={status} onClick={() => setStatusFilter(status)} className="px-3 py-2 rounded-xl text-xs font-medium transition-all capitalize" style={{ background: statusFilter === status ? (STATUS_COLORS[status] || '#3b82f6') : '#f8fafc', color: statusFilter === status ? 'white' : '#64748b', border: '1px solid', borderColor: statusFilter === status ? 'transparent' : '#e2e8f0' }}>
          ...
          <table className="w-full min-w-[1240px]">
            <thead>
              <tr style={{ background: '#f8fafc' }}>
                {['Reference', 'Customer', 'Route', 'Category', 'Fare', 'Affiliate', 'Driver', 'Vehicle', 'Rating', 'Status', ''].map(header => (
              ...
              {filtered.map(job => (
                <tr key={job.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-1.5"><Route size={12} className="text-slate-400" /><span className="text-xs text-slate-600 truncate max-w-32">{job.pickupAddress} to {job.dropoffAddress}</span></div>
                  </td>
                  <td className="px-5 py-4"><span className="text-sm capitalize text-slate-600">{job.vehicleCategory}</span></td>
                  <td className="px-5 py-4"><span className="font-semibold text-slate-800">GBP {job.fareAmount}</span></td>
                  <td className="px-5 py-4">{job.affiliateName ? <p className="text-xs font-medium text-slate-700 max-w-36 truncate" title={job.affiliateName}>{job.affiliateName}</p> : <span className="text-xs text-slate-400">Independent / none</span>}</td>
                  <td className="px-5 py-4">
                    {job.driverName ? (
                      <div>
                        <p className="text-xs font-medium text-slate-700 max-w-32 truncate" title={job.driverName}>{job.driverName}</p>
                        {driverTypeLabel(job.driverType) && <p className="text-[10px] text-slate-400">{driverTypeLabel(job.driverType)}</p>}
                      </div>
                    ) : <span className="text-xs text-slate-400">Unassigned</span>}
                  </td>
                  <td className="px-5 py-4">{job.vehicleLabel ? <p className="text-xs text-slate-600 max-w-36 truncate" title={job.vehicleLabel}>{job.vehicleLabel}</p> : <span className="text-xs text-slate-400">Not allocated</span>}</td>
