      ...
      <div className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-brand-black rounded-3xl p-10 sm:p-14 relative overflow-hidden mb-16">
            ...
            <div className="relative grid lg:grid-cols-2 gap-10 items-center">
              <div>
                ...
                {['20% off all coach bookings', 'Priority booking & 24/7 support', 'Monthly consolidated invoicing', 'Dedicated account manager', 'Flexible payment terms'].map(f => (
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { title: 'Airport transfers', disc: '15% off', desc: 'All inbound and outbound airport transfers with code AIRPORT15.', code: 'AIRPORT15' },
              { title: 'Group bookings', disc: '10% off', desc: 'Groups of 20+ passengers automatically receive a group travel discount.', code: 'AUTO-APPLIED' },
              { title: 'New customers', disc: 'Â£10 off', desc: 'First booking discount for all new Ride Prestige customers.', code: 'WELCOME10' },
            ].map(({ title, disc, desc, code }) => (
              <div key={title} className="bg-brand-grey-pale rounded-2xl p-6">
                <p className="text-brand-gold font-bold text-2xl mb-1">{disc}</p>
                <h4 className="font-semibold text-brand-black mb-2">{title}</h4>
                <p className="text-brand-grey text-sm mb-4">{desc}</p>
                <span className="font-mono text-xs bg-white border border-gray-200 px-3 py-1.5 rounded-lg text-brand-charcoal">{code}</span>
              </div>
            ))}
          </div>
        </div>
