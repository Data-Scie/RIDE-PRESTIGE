...
export default function AdminContentPage() {
  ...
  const toggleSectionVisible = (pageId: string, sectionId: string) => {
    ...
    if (section) {
      updateSection(pageId, { ...section, visible: !section.visible });
    }
  };

  const savePage = () => {
    if (!selectedPage) return;
    setPageList(prev => prev.map(p => p.id === selectedPage.id ? selectedPage : p));
    setSaved(true);
          ...
          {selectedPage ? (
            <>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-6">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={savePage} className="btn-gold flex items-center gap-1.5 py-2 px-4 text-xs">
                      <Save size={13} /> Save
                    </button>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-6">
                ...
                <p className="text-xs text-brand-grey mb-4">Toggle visibility or edit content for each section.</p>
