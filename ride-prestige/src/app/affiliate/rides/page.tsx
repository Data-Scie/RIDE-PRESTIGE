...
export default function AffiliateRidesPage() {
  ...
  const confirmAssign = async () => {
    ...
    try {
      ...
      const accepted = pending.find(r => r.id === assignModal);
      if (accepted) setActive(prev => [...prev, { ...accepted, status: 'driver_assigned' }]);
      setPending(prev => prev.filter(r => r.id !== assignModal));
