  ...
  return (
    <div className="p-3" style={{ borderTop: '1px solid #f1f5f9' }}>
      <div className="flex items-center gap-2.5 px-3 py-2 mb-1">
        <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white" style={{ background: '#10b981' }}>SP</div>
        <div><p className="text-xs font-medium text-slate-700">Sheffield Premier Cars</p><p className="text-xs text-slate-400" style={{ fontSize: '10px' }}>info@sheffieldpremier.co.uk</p></div>
      </div>
    ...
    <aside className="w-64 min-h-screen flex flex-col shrink-0 bg-white" style={{ borderRight: '1px solid #f1f5f9' }}>
      <div className="p-5" style={{ borderBottom: '1px solid #f1f5f9' }}>
        <Link href="/affiliate/dashboard" onClick={onNav} className="flex items-center gap-2.5">
          ...
          <div><p className="font-bold text-slate-800 text-sm" style={{ fontFamily: 'Playfair Display,Georgia,serif' }}>Affiliate Portal</p><p style={{ color: '#10b981', fontSize: '9px', fontWeight: 700, letterSpacing: '0.15em', textTransform: 'uppercase' }}>Sheffield Premier Cars</p></div>
        </Link>
      </div>
      <Suspense fallback={<div className="flex-1" />}><AffNavLinks onNav={onNav} /></Suspense>
      <Suspense fallback={null}><AffLogout /></Suspense>
    </aside>
  );
}
...
function AffiliateLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen" style={{ background: '#f8fafc' }}>
      ...
      {open && <div className="fixed inset-0 z-50 lg:hidden"><div className="absolute inset-0 bg-black/40" onClick={() => setOpen(false)} /><div className="absolute left-0 top-0 bottom-0 w-64"><Sidebar onNav={() => setOpen(false)} /></div></div>}
