...
const NAV = [
  ...
  { label: 'Network', items: [{ href: '/ops/affiliates', label: 'Affiliates', icon: Building2 }, { href: '/ops/drivers', label: 'Independent Drivers', icon: Car }, { href: '/ops/customers', label: 'Customers', icon: Users }] },
    ...
    <aside className="w-64 min-h-screen flex flex-col shrink-0" style={{ background: '#0f172a', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
      <div className="p-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <Link href="/ops/dashboard" onClick={onNav} className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0" style={{ background: 'linear-gradient(135deg,#3b82f6,#1d4ed8)', boxShadow: '0 4px 12px rgba(59,130,246,0.3)' }}><Shield size={16} className="text-white" /></div>
    ...
    <div className="flex min-h-screen" style={{ background: '#f1f5f9' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="lg:hidden flex items-center justify-between px-4 py-3" style={{ background: '#0f172a', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <div className="flex items-center gap-2"><Shield size={16} className="text-blue-400" /><span className="text-white font-bold text-sm">Operations</span></div>
