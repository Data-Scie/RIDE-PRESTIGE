...
import { ArrowLeft, Bus, Car, CheckCircle, Star, Van } from 'lucide-react';
import { useRouter } from 'next/navigation';
import type { QuoteResult } from '@/types';
import { formatCurrency, getCategoryLabel } from '@/lib/fare';

const ICON: Record<string, ReactNode> = { prestige:<Star size={20} />, minibus:<Van size={20} />, coaches:<Bus size={20} />, taxi:<Car size={20} /> };
...
export default function QuoteClient() {
  ...
  useEffect(() => {
    const s = sessionStorage.getItem('rp_quote');
    if (s) setQuote(JSON.parse(s));
  }, []);

  const handleAccept = () => {
    setAccepted(true);
    setTimeout(() => router.push('/thank-you?status=accepted'), 1200);
  };
          ...
          <div className="card p-8">
            ...
            <div className="flex items-start gap-4 mb-6">
              ...
              <div className="flex flex-col justify-between h-14">
                <div>
                  <p className="font-semibold text-sm" style={{color:'#0a0f1e'}}>Pickup postcode</p>
                  <p className="text-xs font-mono" style={{color:'#8b8fa8'}}>{journey.pickupPostcode}</p>
                </div>
                <div>
                  <p className="font-semibold text-sm" style={{color:'#0a0f1e'}}>Drop-off postcode</p>
            ...
            <div className="mt-5 pt-5 border-t-2 rounded-xl p-5 -mx-2" style={{borderColor:'rgba(201,168,76,0.2)',background:'rgba(201,168,76,0.03)'}}>
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  <p className="text-xs italic" style={{color:'rgba(139,143,168,0.7)'}}>This estimate is tailored to your journey and confirmed on booking.</p>
                </div>
          ...
          {!accepted ? (
            <div className="grid sm:grid-cols-2 gap-4">
              <button type="button" onClick={handleAccept}
                className="flex items-center justify-center gap-2 py-4 rounded-xl font-semibold transition-all hover:-translate-y-0.5"
            ...
            <div className="card p-8 text-center">
              ...
              <h3 className="text-2xl font-semibold mb-2" style={{fontFamily:'Playfair Display,Georgia,serif',color:'#0a0f1e'}}>Quote accepted!</h3>
              <p style={{color:'#8b8fa8'}}>Redirecting to confirmation…</p>
            </div>
          )}
